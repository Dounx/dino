    ...
    module Danmu
      class Packet
        ...
        class << self
          ...
          def decode(data)
            {
              protocol: data[Offset::PROTOCOL...Offset::OPERATION].pack("C*").unpack("n").first,
              operation: data[Offset::OPERATION...Offset::SEQUENCE_ID].pack("C*").unpack("N").first,
              sequence_id: data[Offset::SEQUENCE_ID...Offset::BODY].pack("C*").unpack("N").first,
